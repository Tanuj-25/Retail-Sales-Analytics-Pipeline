...
def sales_mart_calculation_table_write(final_sales_team_datamart_df):
    window = Window.partitionBy("store_id", "sales_person_id", "sales_month")
    final_sales_temp_datamart = final_sales_team_datamart_df.\
    ...
    final_sales_team_datamart_table = final_sales_temp_datamart.\
    ...
    # Write the data into the MySQL customer_datamart_table
    print("Writing the data intot the customer_datamart_table")
    db_writer = DatabaseWriter(config.url, config.properties)
    db_writer.write_dataframe(final_sales_team_datamart_table, config.sales_team_data_mart_table)